    ...
    <div class="row navbar" style="" id="nav2">
        <ul class="nav nav-pills  nav-justified" style="background-color: rgb(51,51,51);">
            ...
            <c:forEach items="${newsList}" var="n" begin="0" end="0">
                <li class="active"><a href="/news?nid=${n.newsid}">新闻中心</a></li>
            </c:forEach>
    ...
    <div class="row">
        <div class="col-lg-4">
            <ul class="nav nav-pills nav-stacked" style="padding: 0;margin: 0;border: 0;">
                <c:forEach items="${newsList}" var="n" varStatus="vs">
                    <c:if test="${news.newsid==n.newsid}">
                        <li class="active"><a href="/news?nid=${n.newsid}">${n.head}</a></li>
        ...
        <div class="col-lg-8">
            <div class="buttonBtn" >
                <ul class="pager">
                    <c:if test="${page!=1}">
                        <c:forEach items="${newsList}" var="n" begin="${page-2}" end="${page-2}">
                            <li class="previous"><a href="/news?nid=${n.newsid}">&larr; 上一页</a></li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${page==1}">
                        <c:forEach items="${newsList}" var="n" begin="${page-1}" end="${page-1}">
                            <li class="previous disabled"><a href="/news?nid=${n.newsid}">&larr; 上一页</a></li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${page!=count}">
                        <c:forEach items="${newsList}" var="n" begin="${page}" end="${page}">
                            <li class="next"><a href="/news?nid=${n.newsid}" >下一页&rarr;</a></li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${page==count}">
                        <c:forEach items="${newsList}" var="n" begin="${page-1}" end="${page-1}">
                            <li class="next disabled"><a  href="/news?nid=${n.newsid}">下一页&rarr;</a></li>
                        </c:forEach>
                    </c:if>
                </ul>
            </div>
